...
func _after_phone_outro_narration() -> void:
	...
	var lines = [
		"Danilo contemplates everything that happened that day."
	]
	...
	# Enable bed interaction after this part
...
func switch_location(scene: PackedScene) -> void:
	...
	# Reset dynamic node references
	player_danilo = null
	tip_interact = null
	bed_area = null
	sleep_marker = null

	# === Look for player_danilo ===
...
func _on_open_phone():
	stop_buzz()
	Hud.show_phone_with_unknown_sender()  
	DialogueManager.start_dialogue("start_unknown_chat")  
...
func _on_unknown_sender_calling() -> void:
	...
	self.phone_instance = phone_call_instance
...
func _on_answered_call_narration_panel() -> void:
	# --- Answered call narration ---
	TransitionFade.transition()
	await SignalBus.on_transition_finished

	switch_location(NARRATION_PANEL)  # Or a specific answered call narration panel if different
	await get_tree().process_frame

	var lines = [
		"Danilo tries to make sense of the call, but the voice lingers in his mind.",
		"He feels a strange pull toward the unknown."
	]
...
func _on_rejected_call_narration_panel() -> void:
	...
	switch_location(NARRATION_PANEL)  # Or a separate rejected call panel if needed
	await get_tree().process_frame

	var lines = [
		"Danilo shakes his head, trying to forget the call,",
		"but a lingering unease follows him."
	]
...
func scene_1_done() -> void:
	...
	print("ACT 2 SCENE 1 DONE NA AAAAA...")
